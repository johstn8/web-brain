...
Vorschau-Sperren und produktive Auffindbarkeit sind verschiedene Zustände. Für jede Website führt `release-readiness/<website-slug>.md` nach [[60-operations/release-readiness-register.md]] den Ist- und Zielzustand von nginx `auth_basic`, `X-Robots-Tag`, Robots-Meta, `robots.txt`, Canonicals, Sitemap, Redirects, DNS und Zertifikaten. Eine Seite gilt nicht als indexierbar, nur weil ein einzelnes `noindex` entfernt wurde.
