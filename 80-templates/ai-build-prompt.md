...
```text
Arbeite nach AGENTS.md, beginne bei [[00-start/00-brain-index.md]] und folge [[00-start/05-web-product-workflow.md]]. Bei einem Umsetzungsauftrag lege vor jeder weiteren Arbeit `../projekte/<Projektname>/` mit allen Pflichtdateien an. Baue nie im Brain oder in einem anderen Projektordner.
...
- Wähle eine benannte Auftaktkomposition aus [[20-design/landing-page-craft.md#Auftakt-Repertoire]] und besetze alle sechs Auftaktrollen, insbesondere Beweisanker und Fortschritt. Die Abschnittsfolge entsteht aus den realen Nutzerfragen dieser Zielgruppe, nicht aus der Kette Hero, drei Karten, Logo-Wand, Stimmen, Preise, FAQ.
- Rendere vor der ersten Komponente eine Stilkachel mit Tokenvertrag, Type Ramp, Radius-/Rahmen-/Tiefengrammatik, Aktionszuständen, gewählter Inhaltsgrundform und Signaturdetail, in Licht und Dunkel, an echtem Text dieser Website. Sieh sie dir an und behebe Befunde dort, bevor du einen Auftakt baust.
- Wähle die Komposition nicht gedanklich, sondern am Bild: Baue zwei bis drei Auftaktfassungen mit verschiedenen Kompositionen und denselben realen Inhalten, sieh sie nebeneinander bei 375 und 1280 Pixel an, wähle eine und begründe die Wahl im Design Contract.
- Führe danach je gebauter Website mindestens drei Durchgänge des [[20-design/visual-iteration-loop.md]] am ganzseitigen Render des laufenden Builds: `D1 Komposition`, `D2 Rhythmus und Hierarchie`, `D3 Zustand und Detail`. Sieh dir den Render selbst an, schreibe je Durchgang eine Befundliste mit Ort, Beobachtung und Änderung, und setze sie um. Ein Render ohne Befundliste zählt nicht als Durchgang.
- Wähle die Kopfzeilenform aus [[30-frontend/components-and-ui-states.md#Kopfzeilen-Repertoire]]. Auf 375 Pixel belegt sie höchstens etwa ein Achtel der sichtbaren Höhe, und sie trägt höchstens eine primäre Aktion.
- Setze genau ein hergeleitetes Signaturdetail nach [[20-design/landing-page-craft.md#Das Signaturdetail]] und wiederhole es auf den Unterseiten ruhiger.
- Gehe vor der Abnahme [[20-design/anti-ai-slop.md#Slop-Signaturen]] durch und begründe jede bewusst eingesetzte Signatur im Design Contract.
- Unterseiten übernehmen dasselbe Designsystem, treten aber ruhiger auf.
- Wähle einen Leitbenchmark aus [[20-design/interface-benchmarks.md]] und benenne, was du daraus übernimmst und was ausdrücklich nicht.
- Halte Informationen knapp und logisch gestaffelt. Entscheide die Menge am 375-Pixel-Viewport und halte die Textbudgets aus [[10-strategy/information-density-and-mobile-clarity.md]] ein.
- Überarbeite jedes eingesetzte Bestandsbild vor dem Einsatz: Winkel, Ausschnitt, Hintergrund, Farbe, Auflösung. Stelle Objekte frei und gleiche Bildserien an. Ein Auftakt darf ohne Bild arbeiten, wenn ein anderer konkreter Inhaltsanker ihn trägt. Ist im Design Contract ein Leitbild vorgesehen und fehlt es real, erzeuge ein KI-Bild und setze es ein, ohne es auf der Website zu kennzeichnen; führe es im Asset Register als `ai-placeholder` mit Prompt und Ersetzungshinweis. Kanonisch in [[20-design/imagery-and-ai-editing.md]].
- Lege je Landing Page die stärkste reale Beweisform fest. Baue ein interaktives Kernmodul nach [[20-design/motion-and-interaction.md#Interaktives Kernmodul]] nur, wenn der Nutzer damit etwas Reales besser verstehen, prüfen, wählen oder erleben kann.
- Setze **vor der ersten Komponente** den vollständigen Tokenvertrag nach [[20-design/color-system.md#Tokenvertrag]] in genau einer Tokenquelle, einschließlich `border-hover` und `accent-subtle`. Ohne diese Rollen wirken alle Zustände flach.
- Halte die H0-Untergrenze aus [[20-design/interface-benchmarks.md#H0 Handwerksuntergrenze]] ein. Entscheide Radius, Rahmen, Flächenlogik, Karten-/Zeilen-/Tabellen-/Listenrepertoire, Kopf- und Fußbereich, Chrome, Zweitschrift und Motion **je Website**. B5 ist nur ein wählbares Stilprofil.
...
- Setze pro Website einen eigenen Zeit- und Kurvensatz. Die B5-Werte aus [[20-design/motion-and-interaction.md#Kalibrierte Bewegungsbeispiele]] sind Beispiele, keine Grundeinheit.
...
Vor der Abgabe:
Führe für jede gebaute Website zuerst den [[20-design/visual-iteration-loop.md]] zu Ende und danach den Impeccable KI-Detail-Review nach [[20-design/anti-ai-slop.md#Impeccable KI-Detail-Review]] auf der real laufenden Website durch. Prüfe jede tatsächlich vorkommende Komponenten-/Flächenkombination auf Kontrast sowie width-basierte Kinder in height-basierten Containern auf Geometrie. Suche gezielt nach Details, die nach KI-Generat aussehen, korrigiere sie und dokumentiere Datum, Befunde und Umsetzungsstand.
